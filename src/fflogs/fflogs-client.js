...
export default class FflogsClient{
    ...
    async getToken(){
        const authHeader = 'Basic '+btoa(this.conf.client_id+':'+this.conf.client_secret);
        ...
        if (response.status === 200) {
            this.nextTokenRefreshTime = Math.floor(Date.now() / 1000) + json.expires_in;
